...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Sample car data (replace with your own dataset)
    const usedCars = [
        {
            image: "./images/toyota-silver.avif",
            year: 2018,
            make: "Toyota",
            model: "Camry",
            mileage: 30000,
    ...
    // Function to render the car list based on filters
    function renderCars() {
        const selectedMake = makeFilter.value;
        const selectedPrice = priceFilter.value;
        const minYear = parseInt(document.getElementById("minYear").value) || 2014; // Default to the earliest year
        const maxYear = parseInt(document.getElementById("maxYear").value) || 2020; // Default to the latest year
        ...
        if (filteredCars.length === 0) {
            carsContainer.innerHTML = "<p>No cars found matching the selected filters. Please Try Again</p>";
        } else {
    ...
    document.getElementById("minYear").addEventListener("input", renderCars);
    document.getElementById("maxYear").addEventListener("input", renderCars);
